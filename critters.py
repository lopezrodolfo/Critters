...
class Bear(Critter):
    """
    Representation of a bear critter. Bears can eat, fight, and move in the world.
    They can be either grizzly or not, affecting their color and behavior.
    """

    def __init__(self, location, is_grizzly):
        super().__init__(location)
        self.is_grizzly = is_grizzly
        self.last_move_north = False
    ...
    def get_move(self, neighbors):
        if self.last_move_north:
            self.last_move_north = False
            return Direction.WEST
        else:
            self.last_move_north = True
...
class Cheetah(Critter):
    """
    Representation of a cheetah critter. Cheetahs have a hunger level that affects
    their ability to eat and fight. They move in a random direction after a set
    number of moves.
    """

    def __init__(self, location, hunger):
        super().__init__(location)
        self.eating_time = hunger
        self.current_direction = random.choice(
            [Direction.NORTH, Direction.SOUTH, Direction.EAST, Direction.WEST]
        )
        self.current_direction_num_moves = 0
    ...
    def get_move(self, neighbors):
        ...
        self.current_direction_num_moves += 1

        if self.current_direction_num_moves == 3:
            self.current_direction = random.choice(
                [Direction.NORTH, Direction.SOUTH, Direction.EAST, Direction.WEST]
            )
            self.current_direction_num_moves = 0
    ...
    def __str__(self):
        return str(self.eating_time)


class Lion(Critter):
    """
    Representation of a lion critter. Lions have a fighting strategy that depends
    on whether they have fought recently. They move in a specific pattern based
...
class Torero(Critter):
    """
    Representation of a torero critter. Toreros move randomly and have a chance
    to eat or fight. Their behavior is unpredictable, making them unique among
    critters.
    """
    ...
    def fight(self, opponent):
        return random.choice([Attack.ROAR, Attack.POUNCE, Attack.SCRATCH])

    def get_color(self):
        return "blue"

    def get_move(self, neighbors):
        if self.x > self.world_width // 2:
            return Direction.EAST
        else:
            return random.choice([Direction.SOUTH, Direction.EAST])
...
class World:
    ...
    def __init__(self, width, height, window, food_probability):
        ...
        self.critters = []
        self.open_spots = [(x, y) for x in range(width) for y in range(height)]
    ...
    def get_stats(self, critter_type):
        ...
        """
        return (
            self.num_alive[critter_type],
            self.num_wins[critter_type],
        ...
        for critter in list(self.gestate_time.keys()):
            if self.gestate_time[critter] == 0:
                ...
                if critter in self.pregnant_critters:
                    ...
                    baby = spawn_critter(type(critter).__name__, baby_loc, self)
    ...
    def draw(self):
        ...
        regular_font = Font(family="Arial", size=-10, weight="bold")
        small_font = Font(family="Arial", size=-5, weight="bold")
...
def create_window():
    ...
    global stop_button
    stop_button = Button(controls, text="Stop", state=DISABLED, command=stop_sim_loop)
...
def battle(critter1, critter2):
    ...
    if c1_attack == c2_attack:
        ...
        c2_attack == Attack.FORFEIT
        or c1_attack == Attack.ROAR
        and c2_attack == Attack.SCRATCH
        or c1_attack == Attack.SCRATCH
        and c2_attack == Attack.POUNCE
        or c1_attack == Attack.POUNCE
        and c2_attack == Attack.ROAR
    ):
        return critter1, critter2
    else:
        return critter2, critter1
